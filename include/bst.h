...
public:
    ...
    Node* right;
		Node* left;
		Node(T v) : value(v), count(1), left(nullptr), right(nullptr) {}
	};
private:
	Node* root;
	Node* addNode(Node*, T);
...
template<typename T>
void BST<T>::getFrequencies(Node* root, std::vector<std::pair<T, int>>& frequencies) const {
	if (root == nullptr) {
		return;
	}
	getFrequencies(root->left, frequencies);
	frequencies.push_back(std::make_pair(root->value, root->count));
	getFrequencies(root->right, frequencies);
}

template<typename T>
std::vector<std::pair<T, int>> BST<T>::getFrequencies() {
